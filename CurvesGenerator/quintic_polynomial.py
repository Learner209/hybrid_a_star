"""
***************************************************************************************/
*    Title: zhm-real MotionPlanning Quintic_polynomial implementation
*    Author: zhm-real
*    Date: 2023
*    Availability: https://github.com/zhm-real/MotionPlanning.git
*
***************************************************************************************/
"""
...
class QuinticPolynomial:
    """
    Piecewise QuinticPolynomial commonly used in lattice planner
    """
...
def non_holonomic_simulation(sx, sy, syaw, gx, gy, gyaw, sv = 1, gv = 1, T=100):
    ...
    path = Trajectory()
    xqp = QuinticPolynomial(sx, sv_x, 0, gx, gv_x, 0, T)
    yqp = QuinticPolynomial(sy, sv_y, 0, gy, gv_y, 0, T)
    ...
    if VIS_PLOT:
        ...
        plt.show()
    return np.array(path.x), np.array(path.y), np.array(path.v), np.array(path.yaw)


def simulation():
    sx, sy, syaw, sv, sa = 5.0, 5.0, np.deg2rad(120.0), 1.0, 0.0
    gx, gy, gyaw, gv, ga = 110.0, 110.0, np.deg2rad(90.0), 1.0, 0.0

    MAX_ACCEL = 0.0  # max accel [m/s2]
    MAX_JERK = 0.0 # max jerk [m/s3]
    dt = 0.1  # T tick [s]
...
if __name__ == '__main__':
    non_holonomic_simulation(sx=10.0, sy=10.0, syaw=np.deg2rad(120), gx=110, gy=110, gyaw=np.deg2rad(180), sv = 1, gv = 1)
    # simulation()
